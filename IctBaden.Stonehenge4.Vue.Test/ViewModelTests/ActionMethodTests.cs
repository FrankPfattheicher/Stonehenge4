...
public sealed class ActionMethodTests : IDisposable
{
    ...
    private async Task ExecutePost(string parameterValue)
    {
        ...
        try
        {
            ...
            startVm = await client.DownloadStringWithSession(_app.BaseUrl + "/ViewModel/StartVm").ConfigureAwait(StonehengeGlobal.ConfigureAwait);
            response = await client.Post(_app.BaseUrl + "/ViewModel/StartVm/TestAction?parameter=" + parameterValue, string.Empty).ConfigureAwait(StonehengeGlobal.ConfigureAwait);
        }
        catch (Exception ex)
        {
