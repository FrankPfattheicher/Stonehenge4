...
public partial class RedirectableHttpClient : HttpClient
{
    ...
    public async Task<string> DownloadStringWithSession(string address)
    {
        if (SessionId == null)
        {
            ...
            await DownloadString(indexUrl.ToString()).ConfigureAwait(Program.ConfigureAwait);
        }
        ...
        return await DownloadString(url.ToString()).ConfigureAwait(Program.ConfigureAwait);
    }
    ...
    public async Task<string> DownloadString(string address)
    {
        ...
        {
            using var response = await GetAsync(address).ConfigureAwait(Program.ConfigureAwait);
    ...
    public async Task<string> Post(string address, string data)
    {
        ...
        using var response = await PostAsync(address, content).ConfigureAwait(Program.ConfigureAwait);
        var body = response.Content.ReadAsStringAsync().Result;
        return body;
    }
    ...
    {
        using var content = new StringContent(data);
        using var response = await PutAsync(address, content).ConfigureAwait(Program.ConfigureAwait);
        var body = response.Content.ReadAsStringAsync().Result;
        return body;
    }
    ...
    {
        using var response = await DeleteAsync(address).ConfigureAwait(Program.ConfigureAwait);
        var body = response.Content.ReadAsStringAsync().Result;
        return body;
    }
