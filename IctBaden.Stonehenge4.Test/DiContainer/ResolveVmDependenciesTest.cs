...
public sealed class ResolveVmDependenciesTest : IDisposable
{
    ...
    {
        Id = Guid.NewGuid();
        await _loader.Get(_session, CancellationToken.None, _loader, "ViewModel/" + nameof(TestSimpleVmWithDependency), new Dictionary<string, string>(StringComparer.Ordinal));
    ...
    {
        Id = Guid.NewGuid();
        await _loader.Get(_session, CancellationToken.None, _loader, "ViewModel/" + nameof(TestActiveVmWithDependency), new Dictionary<string, string>(StringComparer.Ordinal));
