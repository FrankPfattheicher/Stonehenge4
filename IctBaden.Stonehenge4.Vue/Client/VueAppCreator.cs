...
internal class VueAppCreator
{
    ...
    private async Task<string> LoadResourceText(string resourceName)
    {
        var resource = await _loader.Get(AppSession.None, CancellationToken.None, _loader, resourceName,
            new Dictionary<string, string>(StringComparer.Ordinal)).ConfigureAwait(Program.ConfigureAwait);
    ...
    public async Task<List<string>> CreateElements()
    {
        ...
        foreach (var element in customElements)
        {
            try
            {
                ...
                var template = await LoadResourceText($"{source}.html").ConfigureAwait(Program.ConfigureAwait);
                ...
                var methods = await LoadResourceText($"{source}.js").ConfigureAwait(Program.ConfigureAwait);
